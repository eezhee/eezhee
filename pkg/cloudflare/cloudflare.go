...
func Test(apiToken string) bool {
	...
	clusterDNSRecord := eezhee.HostInfo{Name: clusterName, IP: "218.1.2.3"}
...
func (m *DNSManager) GetHostRecord(hostname string) (*eezhee.HostInfo, error) {
	...
	// found a record
	hostInfo := eezhee.HostInfo{Name: recs[0].Name, IP: recs[0].Content, ID: recs[0].ID}

	return &hostInfo, nil
}

// AddHostRecord will get details of dns record for a given host
func (m *DNSManager) AddHostRecord(hostInfo eezhee.HostInfo) error {
...
func (m *DNSManager) UpdateHostRecord(hostInfo eezhee.HostInfo) error {
...
func (m *DNSManager) DeleteHostRecord(hostInfo eezhee.HostInfo) error {
